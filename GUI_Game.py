...
title = canvas.create_text(
    300, 200, text='The Candy Monster', fill='black', font=('Century725 Cn BT', 20))
directions = canvas.create_text(
    300, 300, text='Eat candy but avoid the red ones', fill='red', font=('Century725 Cn BT', 15))
...
def make_candy():
    ...
    candy = canvas.create_oval(
        xposition, 0, xposition + 20, 20, fill=candy_color)
...
def check_hits():
    for candy in bad_candy_list:
        if collision(mychar, candy, 30):
            game_over = canvas.create_text(
                300, 300, text='Game Over', fill='red', font=('Century725 Cn BT', 30))
